...
exports.sendVerificationCode = async (req, res) => {
  try {
    ...
    console.log(
      `Type of verification code from randomize ${typeof verificationCode}`
    );
    ...
    if (userExist && !userExist.verified) {
      ...
      res
        .status(200)
        .json({ message: "Verification code sent. Please check your email." });
...
exports.confirmVerificationCode = async (req, res) => {
  ...
  try {
    ...
    console.log(
      `Type of code code to confirm from req.body${typeof verificationCode}`
    );
...
exports.registerUser = async (req, res) => {
  ...
  try {
    const { fullName, registrationNumber, password, confirmPassword } =
      req.body;
